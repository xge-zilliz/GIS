...
template
class ChoroplethMap<int8_t>;

template
class ChoroplethMap<int16_t>;

template
class ChoroplethMap<int32_t>;

template
class ChoroplethMap<int64_t>;

template
class ChoroplethMap<uint8_t>;

template
class ChoroplethMap<uint16_t>;

template
class ChoroplethMap<uint32_t>;

template
class ChoroplethMap<uint64_t>;

template
class ChoroplethMap<float>;

template
class ChoroplethMap<double>;

template<typename T>
ChoroplethMap<T>::ChoroplethMap(std::shared_ptr<std::string> choropleth_wkt,
...
ChoroplethMap<T>::Transform() {
    ...
    for (int i = 0; i < num_buildings_; i++) {
        ...
        OGRGeometryFactory::createFromWkt(choropleth_wkt_.get()[i].c_str(), nullptr, &geometry);
...
ChoroplethMap<T>::SetColor() {
    ...
    for (auto i = 0; i < num_buildings_; i++) {
        auto count = count_.get()[i] >= count_start ? count_.get()[i] : count_start;
        count = count_.get()[i] <= count_end ? count : count_end;
        auto ratio = (count - count_start) / count_range;
        auto circle_params_2d = ColorGradient::GetCircleParams(choropleth_vega_.color_style(), ratio);
