...
#include "vega_choropleth_map.h"

namespace zilliz {
namespace render {

VegaChoroplethMap::VegaChoroplethMap(const std::string &json) {
    Parse(json);
}

std::string
VegaChoroplethMap::Build() {
...
void
VegaChoroplethMap::Parse(const std::string &json) {
    ...
    for (int i = 0; i < 4; i++) {
        if (!JsonTypeCheck(mark_enter["bounding_box"]["value"][i], rapidjson::Type::kNumberType)) {
            return;
        }
    }
    ...
    ruler_ = std::make_pair(mark_enter["ruler"]["value"][0].GetDouble(), mark_enter["ruler"]["value"][1].GetDouble());
